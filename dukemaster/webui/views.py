...
from dukemaster.utils import is_float, get_signature, parse_auth_header, json
from dukemaster.commands import call_command
...
def command(request):
    if request.META.get('HTTP_AUTHORIZATION', '').startswith('Duke'):
        ...
        if signature and timestamp:
            ...
            sig_hmac = get_signature('f30j4f08q3j4f98q34f93hq4', data, timestamp)
            if sig_hmac != signature:
                return HttpResponseForbidden('Invalid signature')
        else:
             return HttpResponse('Unauthorized', status_code=401)
    else:
         return HttpResponseForbidden('Invalid signature2')
